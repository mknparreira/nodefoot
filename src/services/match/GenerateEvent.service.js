...
const MatchConfig = globalConfig.get('EventMatch');
...
class GenerateEvent {
  ...
  calcuateLesion() {
    ...
    if (minute <= 15) {
      eventType.probability = (MatchConfig.veryLow / 100) * 100;
      return eventType;
    }
    ...
    if (minute >= 46 && minute <= 70) {
      eventType.probability = (MatchConfig.low / 100) * 100;
      return eventType;
    }

    if (minute >= 71 && minute <= 90) {
      eventType.probability = (MatchConfig.high / 100) * 100;
      return eventType;
    }
  ...
  calculateGoal() {
    ...
    if (minute >= 16 && minute <= 30) {
      eventType.probability = (MatchConfig.normal / 100) * 100;
      return eventType;
    }
